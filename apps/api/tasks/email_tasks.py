...
def send_magic_code_email(self, to_email: str, code: str, expiry_minutes: int = 10):
    ...
    try:
        subject = f"Your Studio login code: {code}"
        ...
        text_body = f"Your Studio login code is: {code}. This code expires in {expiry_minutes} minutes."
...
def send_invite_email(
    ...
    try:
        subject = f"You've been invited to join {org_name} on Studio"
        html_body = render_template(
            "email/invite.html",
            subject=subject,
        ...
        text_body = f"{inviter_name} has invited you to join {org_name} on Studio. Accept here: {invite_link}"
